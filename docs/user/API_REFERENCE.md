...
> **Rust binding note:** `ParquetReader::schema()` in the Rust crate returns `SchemaRef<'_>` (a
> lifetime-bound view, not `Schema`). The lifetime annotation ties the returned reference to
> the reader, preventing use-after-free at compile time. Do not attempt to store `SchemaRef`
> beyond the owning `ParquetReader`'s scope — the borrow checker enforces this automatically.
