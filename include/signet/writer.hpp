...
[[nodiscard]] inline std::vector<uint8_t> wrap_encrypted_page_header(
        const std::vector<uint8_t>& encrypted_header) {
    std::vector<uint8_t> out;
    out.reserve(8 + encrypted_header.size());
    out.insert(out.end(), std::begin(kEncryptedPageHeaderMagic),
               std::end(kEncryptedPageHeaderMagic));
    const uint32_t size = static_cast<uint32_t>(encrypted_header.size());
    out.push_back(static_cast<uint8_t>(size & 0xFFu));
    out.push_back(static_cast<uint8_t>((size >> 8) & 0xFFu));
    out.push_back(static_cast<uint8_t>((size >> 16) & 0xFFu));
    out.push_back(static_cast<uint8_t>((size >> 24) & 0xFFu));
    out.insert(out.end(), encrypted_header.begin(), encrypted_header.end());
    return out;
}
...
struct WriterOptions {
    ...
    // -- Encryption (AGPL-3.0+ project, commercial-enabled build gate) -------
#if defined(SIGNET_ENABLE_COMMERCIAL) && SIGNET_ENABLE_COMMERCIAL
...
public:
    ...
    [[nodiscard]] expected<void> write_column(size_t col_index,
        ...
        // Validate that column is BYTE_ARRAY or FIXED_LEN_BYTE_ARRAY
        const PhysicalType actual_pt = schema_.column(col_index).physical_type;
        ...
        // CWE-130: For FIXED_LEN_BYTE_ARRAY columns, each string must be
        // written as raw bytes (no length prefix) via write_fixed_len_byte_array
        // to produce a valid column chunk. The ColumnWriter validates that
        // each value's length matches the schema's type_length.
        if (actual_pt == PhysicalType::FIXED_LEN_BYTE_ARRAY) {
            for (size_t i = 0; i < count; ++i) {
                col_writers_[col_index].write_fixed_len_byte_array(
    ...
    [[nodiscard]] expected<void> flush_row_group() {
        ...
        for (size_t c = 0; c < col_writers_.size(); ++c) {
            ...
            if (col_encoding == Encoding::PLAIN_DICTIONARY ||
                ...
                const auto& raw_header_bytes = header_enc.data();

                const uint8_t* header_write_data = raw_header_bytes.data();
                size_t header_write_size = raw_header_bytes.size();
#if defined(SIGNET_ENABLE_COMMERCIAL) && SIGNET_ENABLE_COMMERCIAL
                std::vector<uint8_t> header_record_buf;
                if (encryptor_ && encryptor_->is_column_encrypted(col_desc.name)) {
                    auto enc_header = encryptor_->encrypt_data_page_header(
                        raw_header_bytes.data(), raw_header_bytes.size(),
                        col_desc.name, static_cast<int32_t>(row_groups_.size()), 0);
                    if (!enc_header) return enc_header.error();
                    header_record_buf = detail::writer::wrap_encrypted_page_header(*enc_header);
                    header_write_data = header_record_buf.data();
                    header_write_size = header_record_buf.size();
                }
#endif

                data_page_offset = file_offset_;

                write_raw(header_write_data, header_write_size);
                write_raw(page_data, page_data_size);

                total_uncompressed = static_cast<int64_t>(header_write_size) +
                                     static_cast<int64_t>(uncompressed_size);
                total_compressed   = static_cast<int64_t>(header_write_size) +
                ...
                if (!col_index_builders_.empty()) {
                    ...
                    builder.set_page_location(data_page_offset,
                        compressed_size + static_cast<int32_t>(header_write_size));
    ...
    [[nodiscard]] expected<WriteStats> close() {
        ...
        if (encryptor_ && encryptor_->config().encrypt_footer) {
            ...
            // Signed plaintext footer mode (PARQUET-1178 §4.2): footer is
            // readable but HMAC-signed for tamper detection.
            auto signed_footer = encryptor_->sign_footer(
                footer_bytes.data(), footer_bytes.size());
            if (!signed_footer) {
                file_.close();
                open_ = false;
                return signed_footer.error();
            }
            const auto& sf = *signed_footer;

            // Write signed footer (plaintext + 32-byte HMAC appended)
            write_raw(sf.data(), sf.size());

            // Footer length includes the HMAC signature
            uint32_t footer_len = static_cast<uint32_t>(sf.size());
            write_raw_le32(footer_len);

            // PAR1 magic — footer is plaintext (signed, not encrypted)
...
private:
        ...
        for (size_t c = 0; c < schema_.num_columns(); ++c) {
            ...
            for (const auto& row : pending_rows_) {
                ...
                case PhysicalType::FIXED_LEN_BYTE_ARRAY: {
                    const auto& cd = schema_.column(c);
                    if (cd.type_length > 0 &&
                        val.size() != static_cast<size_t>(cd.type_length)) {
                        return Error{ErrorCode::SCHEMA_MISMATCH,
                            "FIXED_LEN_BYTE_ARRAY column '" + cd.name +
                            "': CSV value length " + std::to_string(val.size()) +
                            " != type_length " + std::to_string(cd.type_length)};
                    }
    ...
    [[nodiscard]] expected<DictColumnResult> write_dictionary_column(
        ...
        if (encryptor_ && encryptor_->is_column_encrypted(col_desc.name)) {
            auto enc_result = encryptor_->encrypt_dict_page(
        ...
        dict_ph.crc = static_cast<int32_t>(detail::writer::page_crc32(
            dict_write_data, dict_write_size));
        ...
        idx_ph.crc = static_cast<int32_t>(detail::writer::page_crc32(
            idx_write_data, idx_write_size));
        ...
        const auto& raw_idx_header_bytes = idx_header_enc.data();

        const uint8_t* idx_header_write_data = raw_idx_header_bytes.data();
        size_t idx_header_write_size = raw_idx_header_bytes.size();
#if defined(SIGNET_ENABLE_COMMERCIAL) && SIGNET_ENABLE_COMMERCIAL
        std::vector<uint8_t> idx_header_record_buf;
        if (encryptor_ && encryptor_->is_column_encrypted(col_desc.name)) {
            auto enc_header = encryptor_->encrypt_data_page_header(
                raw_idx_header_bytes.data(), raw_idx_header_bytes.size(),
                col_desc.name, static_cast<int32_t>(row_groups_.size()), 0);
            if (!enc_header) return enc_header.error();
            idx_header_record_buf = detail::writer::wrap_encrypted_page_header(*enc_header);
            idx_header_write_data = idx_header_record_buf.data();
            idx_header_write_size = idx_header_record_buf.size();
        }
#endif

        info.data_page_offset = file_offset_;

        write_raw(idx_header_write_data, idx_header_write_size);
        write_raw(idx_write_data, idx_write_size);

        info.total_uncompressed += static_cast<int64_t>(idx_header_write_size) +
                                   static_cast<int64_t>(idx_uncompressed_size);
        info.total_compressed   += static_cast<int64_t>(idx_header_write_size) +
