...
public:
    ...
    [[nodiscard]] expected<std::vector<uint8_t>> encrypt(
        ...
        auto mac_key = derive_aad_mac_key();
        auto tag = compute_aad_tag(mac_key, aad, iv.data(), iv.size(),
                                   ct_result->data(), ct_result->size());

        std::vector<uint8_t> out;
        out.reserve(1 + iv.size() + ct_result->size() + tag.size());
        out.push_back(static_cast<uint8_t>(iv.size()) | AAD_TAGGED_IV_FLAG);
        out.insert(out.end(), iv.begin(), iv.end());
        out.insert(out.end(), ct_result->begin(), ct_result->end());
    ...
    [[nodiscard]] expected<std::vector<uint8_t>> decrypt(
        ...
        if (size < 1) {
            return Error{ErrorCode::ENCRYPTION_ERROR,
                         "AesCtrCipher: encrypted data too short"};
        }

        const uint8_t iv_flag = data[0];
        const bool tagged = (iv_flag & AAD_TAGGED_IV_FLAG) != 0;
        if (tagged) {
            const size_t iv_size = static_cast<size_t>(iv_flag & ~AAD_TAGGED_IV_FLAG);
            const size_t header_size = 1 + iv_size;
            if (iv_size == 0 || iv_size > AesCtr::IV_SIZE || size < header_size + AAD_TAG_SIZE) {
                return Error{ErrorCode::ENCRYPTION_ERROR,
                             "AesCtrCipher: invalid tagged CTR payload"};
            }
            if (aad.empty()) {
                return Error{ErrorCode::ENCRYPTION_ERROR,
                             "AesCtrCipher: tagged CTR payload requires AAD"};
            }

            const uint8_t* iv = data + 1;
            const size_t ct_size = size - header_size - AAD_TAG_SIZE;
            const uint8_t* ciphertext = data + header_size;
            const uint8_t* stored_tag = ciphertext + ct_size;

            auto mac_key = derive_aad_mac_key();
            auto expected_tag = compute_aad_tag(mac_key, aad, iv, iv_size, ciphertext, ct_size);
            uint8_t diff = 0;
            for (size_t i = 0; i < expected_tag.size(); ++i) {
                diff |= static_cast<uint8_t>(stored_tag[i] ^ expected_tag[i]);
            }
            if (diff != 0) {
                return Error{ErrorCode::ENCRYPTION_ERROR,
                             "AesCtrCipher: AAD authentication failed"};
            }
...
private:
    [[nodiscard]] std::array<uint8_t, 32> derive_aad_mac_key() const {
        static constexpr uint8_t kInfo[] = "signet-aes-ctr-aad-mac-v1";
        auto prk = hkdf_extract(nullptr, 0, key_.data(), key_.size());
        std::array<uint8_t, 32> mac_key{};
        (void)hkdf_expand(prk, kInfo, sizeof(kInfo) - 1, mac_key.data(), mac_key.size());
        return mac_key;
    }

    [[nodiscard]] static std::array<uint8_t, AAD_TAG_SIZE> compute_aad_tag(
        ...
        mac_input.reserve(aad.size() + iv_size + ct_size);
        mac_input.insert(mac_input.end(), aad.begin(), aad.end());
        mac_input.insert(mac_input.end(), iv, iv + iv_size);
        mac_input.insert(mac_input.end(), ciphertext, ciphertext + ct_size);
        return detail::hkdf::hmac_sha256(mac_key.data(), mac_key.size(),
