...
/// Legacy environment variable name for ad-hoc tier overrides (unused in signed-token enforcement).
...
///
/// Populated by `resolve_policy()` from the verified signed commercial license
/// token. When `evaluation_mode` is false, all limit fields are ignored and
/// the library operates without evaluation-tier usage restrictions.
///
/// @see resolve_policy()
...
/// Return the vendor-controlled usage-state file path.
///
/// The path is always derived from the installation context and is never
/// influenced by caller-controlled environment variables. Allowing callers to
/// select the metering file via an environment variable permitted evaluation
/// counter resets by rotating the file path across process restarts (CWE-284).
...
/// Detect the current runtime node/hostname identity from the OS, sanitized.
...
/// Resolve the effective LicensePolicy from the verified signed license token.
///
/// Reads `SIGNET_COMMERCIAL_LICENSE_KEY`, verifies it with the embedded Ed25519
/// public key, and derives evaluation-tier limits from the verified token.
/// Non-evaluation licenses return an empty (unrestricted) policy.
///
...
/// 1. `SIGNET_ENABLE_COMMERCIAL` must be defined and non-zero.
/// 2. The runtime environment variable `SIGNET_COMMERCIAL_LICENSE_KEY` must be
///    present and must verify as a signed Ed25519-backed license token.
///
