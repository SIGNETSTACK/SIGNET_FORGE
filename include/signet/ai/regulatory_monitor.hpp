...
#error "signet/ai/regulatory_monitor.hpp requires SIGNET_ENABLE_COMMERCIAL=ON (AGPL-3.0 commercial tier). See LICENSE_COMMERCIAL."
#endif
...
public:
    ...
    explicit RegulatoryChangeMonitor(const std::string& org_id = "")
        : org_id_(org_id) {
        auto gate = commercial::require_feature("RegulatoryChangeMonitor");
