...
public:
    ...
    [[nodiscard]] expected<void> flush_async() noexcept {
        if (!ptr_) return {};
#if defined(_WIN32)
        if (!FlushViewOfFile(ptr_, size_)) {
            return Error{ErrorCode::IO_ERROR,
                         "MappedSegment: FlushViewOfFile failed for '" + path_ + "'"};
        }
#else
        if (::msync(ptr_, size_, MS_ASYNC) != 0) {
            return Error{ErrorCode::IO_ERROR,
                         std::string("MappedSegment: msync(MS_ASYNC) failed for '") +
    ...
    [[nodiscard]] expected<void> flush_sync() noexcept {
        if (!ptr_) return {};
#if defined(_WIN32)
        if (!FlushViewOfFile(ptr_, size_)) {
            return Error{ErrorCode::IO_ERROR,
        ...
        if (hFile_ != INVALID_HANDLE_VALUE && !FlushFileBuffers(hFile_)) {
            return Error{ErrorCode::IO_ERROR,
                         "MappedSegment: FlushFileBuffers failed for '" + path_ + "'"};
        }
#else
        if (::msync(ptr_, size_, MS_SYNC) != 0) {
            return Error{ErrorCode::IO_ERROR,
                         std::string("MappedSegment: msync(MS_SYNC) failed for '") +
                         path_ + "': " + std::strerror(errno)};
        }
#endif
        return {};
...
public:
    ...
    [[nodiscard]] expected<int64_t> append(const uint8_t* data, size_t size) {
        ...
        if (opts_.sync_on_append) {
            auto flush_result = active->seg.flush_async();
    ...
    [[nodiscard]] expected<void> flush(bool do_sync = false) {
        ...
        if (do_sync || opts_.sync_on_flush)
            return active.seg.flush_sync();
        return active.seg.flush_async();
    }
    ...
    [[nodiscard]] expected<void> close() {
        ...
        for (auto& sp : ring_) {
            ...
            if (st == SlotState::ACTIVE) {
                auto flush_result = sp->seg.flush_sync();
                if (!flush_result && !first_error.has_value()) {
                    first_error = flush_result.error();
                }
            }
            sp->seg.close();
        }
...
private:
    ...
    expected<void> allocate_slot(RingSlot& slot, uint64_t segment_id, bool skip_prefault) {
        ...
        auto flush_result = slot.seg.flush_async();
        if (!flush_result) return flush_result.error();
    ...
    expected<void> rotate() {
        ...
        auto flush_result = old_slot.seg.flush_async();
        if (!flush_result) return flush_result.error();
