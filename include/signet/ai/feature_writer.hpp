...
public:
    ...
    [[nodiscard]] static expected<FeatureWriter> create(Options opts) {
        ...
        // file_prefix and group.name are concatenated into output filenames;
        // reject path separators and traversal components to prevent writes
        // outside the validated output_dir.
        auto has_path_chars = [](const std::string& s) {
            return s.find('/') != std::string::npos ||
                   s.find('\\') != std::string::npos ||
                   s.find("..") != std::string::npos ||
                   s.find('\0') != std::string::npos;
        };
        if (has_path_chars(opts.file_prefix))
            return Error{ErrorCode::IO_ERROR,
                         "FeatureWriter: file_prefix contains path separator or traversal"};
        if (has_path_chars(opts.group.name))
            return Error{ErrorCode::IO_ERROR,
                         "FeatureWriter: group.name contains path separator or traversal"};
