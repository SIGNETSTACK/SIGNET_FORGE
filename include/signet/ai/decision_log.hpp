...
#error "signet/ai/decision_log.hpp requires SIGNET_ENABLE_COMMERCIAL=ON (AGPL-3.0 commercial tier). See LICENSE_COMMERCIAL."
#endif
    ...
    [[nodiscard]] inline expected<void> validate_loaded_columns() const {
        ...
        const auto mismatch = [&](const char* column_name, size_t actual_rows)
            ...
            return Error{
                ...
                "DecisionLogReader: column '" + std::string(column_name) +
                    "' row count mismatch in '" + path_ + "' (expected " +
                    std::to_string(expected_rows) + ", got " +
                    std::to_string(actual_rows) + ")"};
        };

        if (col_strategy_id_.size() != expected_rows)
            return mismatch("strategy_id", col_strategy_id_.size());
        if (col_model_version_.size() != expected_rows)
            return mismatch("model_version", col_model_version_.size());
        if (col_decision_type_.size() != expected_rows)
            return mismatch("decision_type", col_decision_type_.size());
        if (col_input_features_.size() != expected_rows)
            return mismatch("input_features", col_input_features_.size());
        if (col_model_output_.size() != expected_rows)
            return mismatch("model_output", col_model_output_.size());
        if (col_confidence_.size() != expected_rows)
            return mismatch("confidence", col_confidence_.size());
        if (col_risk_result_.size() != expected_rows)
            return mismatch("risk_result", col_risk_result_.size());
        if (col_order_id_.size() != expected_rows)
            return mismatch("order_id", col_order_id_.size());
        if (col_symbol_.size() != expected_rows)
            return mismatch("symbol", col_symbol_.size());
        if (col_price_.size() != expected_rows)
            return mismatch("price", col_price_.size());
        if (col_quantity_.size() != expected_rows)
            return mismatch("quantity", col_quantity_.size());
        if (col_venue_.size() != expected_rows)
            return mismatch("venue", col_venue_.size());
        if (col_notes_.size() != expected_rows)
            return mismatch("notes", col_notes_.size());
