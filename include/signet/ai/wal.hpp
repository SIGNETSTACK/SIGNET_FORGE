...
public:
    ...
    ///         an invalid header.
    static expected<WalReader> open(
        ...
        return WalReader(f, path, WAL_FILE_HDR_SIZE, mode);
    }
    ...
    [[nodiscard]] expected<std::optional<WalEntry>> next() {
        ...
        if (n < 24)
            return soft_stop_or_error("truncated record header");

        uint32_t magic   = detail::read_le32(hdr);
        if (magic != WAL_RECORD_MAGIC)
            return soft_stop_or_error("bad record magic");
        ...
        if (data_sz > WAL_MAX_RECORD_SIZE) {
            return soft_stop_or_error("record size exceeds WAL_MAX_RECORD_SIZE");
        }
        ...
        if (data_sz > 0) {
            ...
            if (nr < data_sz) {
                ...
                return soft_stop_or_error("truncated payload");
            }
        }
        ...
        if (std::fread(crc_buf, 1, 4, file_) != 4)
            return soft_stop_or_error("truncated record CRC");
        ...
        if (crc != stored_crc)
            return soft_stop_or_error("record CRC mismatch");
...
private:
    [[nodiscard]] expected<std::optional<WalEntry>> soft_stop_or_error(
            const char* reason) const {
        if (validation_mode_ == ValidationMode::Strict) {
            return Error{ErrorCode::CORRUPT_DATA,
                         std::string("WalReader: ") + reason +
                         " in '" + path_ + "' at offset " + std::to_string(offset_)};
        }
        return std::optional<WalEntry>{std::nullopt};
    }

    WalReader(FILE* f, std::string path, size_t initial_offset, ValidationMode mode)
        : file_(f), path_(std::move(path)), offset_(initial_offset), validation_mode_(mode) {}

    FILE*       file_     = nullptr;
    std::string path_;
    int64_t     last_seq_ = -1;
    ValidationMode validation_mode_ = ValidationMode::BestEffort;
    int64_t     count_    = 0;
...
struct WalManagerOptions {
    ...
    bool        strict_replay  = true;                 ///< Fail closed on corrupt/unreadable segments during replay.
...
public:
    ...
    static expected<WalManager> open(const std::string& dir, Options opts = {}) {
        ...
        // CWE-22: file_prefix is concatenated into segment filenames —
        // reject path separators and traversal to prevent directory escape.
        {
            const auto& pfx = opts.file_prefix;
            if (pfx.find('/') != std::string::npos ||
                pfx.find('\\') != std::string::npos ||
                pfx.find("..") != std::string::npos ||
                pfx.find('\0') != std::string::npos)
                return Error{ErrorCode::IO_ERROR,
                             "WalManager: file_prefix contains path separator or traversal"};
        }
        ...
        if (!existing.empty()) {
            auto rd = WalReader::open(existing.back(),
                opts.strict_replay ? WalReader::ValidationMode::Strict
                                   : WalReader::ValidationMode::BestEffort);
            if (!rd) {
                if (opts.strict_replay) return rd.error();
            } else {
                auto entries = rd->read_all();
                if (!entries) {
                    if (opts.strict_replay) return entries.error();
        ...
        for (const auto& seg : existing) {
            ...
            if (!rd) {
                ...
                if (!res) {
                    if (opts.strict_replay) return res.error();
                } else {
                    total += static_cast<int64_t>(res->size());
                }
            }
        }
    ...
    [[nodiscard]] expected<void> close() {
        ...
        closed_ = true;
        if (writer_) {
            auto close_result = writer_->close();
    ...
    [[nodiscard]] expected<std::vector<WalEntry>> read_all() {
        ...
        if (writer_ && !closed_) {
            auto flush_result = writer_->flush(false);
            if (!flush_result) return flush_result.error();
        }
        std::vector<WalEntry> result;
        for (const auto& seg : segments_) {
            auto rd = WalReader::open(seg,
                opts_.strict_replay ? WalReader::ValidationMode::Strict
                                    : WalReader::ValidationMode::BestEffort);
            if (!rd) {
                if (opts_.strict_replay) return rd.error();
                continue;
            }
            auto entries = rd->read_all();
            if (!entries) {
                if (opts_.strict_replay) return entries.error();
                continue;
            }
            for (auto& e : *entries)
