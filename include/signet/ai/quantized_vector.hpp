...
public:
    ...
    explicit inline Quantizer(QuantizationParams params)
        ...
        , inv_scale_((std::isfinite(params_.scale) && params_.scale > 0.0f)
                         ? 1.0f / params_.scale
                         : 1.0f) {
        if (!std::isfinite(params_.scale) || params_.scale <= 0.0f) {
            params_.scale = 1.0f;
            params_.zero_point = 0.0f;
        }
    }
...
inline expected<QuantizationParams> QuantizationParams::deserialize(const std::string& s) {
    ...
    if (!std::isfinite(p.scale) || p.scale <= 0.0f) {
        return Error{ErrorCode::INVALID_FILE,
                     "quantization params: scale must be finite and positive"};
    }
    if (!std::isfinite(p.zero_point)) {
        return Error{ErrorCode::INVALID_FILE,
                     "quantization params: zero_point must be finite"};
    }
