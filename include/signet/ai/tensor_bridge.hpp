...
public:
    ...
    [[nodiscard]] T& at(int64_t i) {
        ...
        if (byte_stride_ != 0) {
            auto* base = static_cast<uint8_t*>(data_);
            return *reinterpret_cast<T*>(base + static_cast<size_t>(i) * byte_stride_);
        }
        return typed_data<T>()[i];
    }
    ...
    [[nodiscard]] const T& at(int64_t i) const {
        if (data_ == nullptr || i < 0 || i >= num_elements())
            throw std::out_of_range("TensorView::at(i): index out of range");
        if (byte_stride_ != 0) {
            const auto* base = static_cast<const uint8_t*>(data_);
            return *reinterpret_cast<const T*>(base + static_cast<size_t>(i) * byte_stride_);
        }
        return typed_data<T>()[i];
    }
    ...
    [[nodiscard]] T& at(int64_t row, int64_t col) {
        ...
        if (byte_stride_ != 0) {
            auto* base = static_cast<uint8_t*>(data_);
            auto* row_ptr = reinterpret_cast<T*>(base + static_cast<size_t>(row) * byte_stride_);
            return row_ptr[col];
        }
        return typed_data<T>()[row * cols + col];
    }
    ...
    [[nodiscard]] const T& at(int64_t row, int64_t col) const {
        ...
        if (byte_stride_ != 0) {
            const auto* base = static_cast<const uint8_t*>(data_);
            const auto* row_ptr = reinterpret_cast<const T*>(
                base + static_cast<size_t>(row) * byte_stride_);
            return row_ptr[col];
        }
        return typed_data<T>()[row * cols + col];
    }
