...
#error "signet/ai/compliance/eu_ai_act_reporter.hpp requires SIGNET_ENABLE_COMMERCIAL=ON (AGPL-3.0 commercial tier). See LICENSE_COMMERCIAL."
#endif
...
public:
    ...
    [[nodiscard]] static expected<ComplianceReport> generate_article12(
        ...
        for (const auto& path : inference_log_files) {
            ...
            if (!all) {
                if (opts.strict_source_reads) {
                    return Error{ErrorCode::IO_ERROR,
                        "EUAIActReporter Art.12: failed to read '" + path +
                        "': " + all.error().message};
                }
                ++read_errors;
                continue;
    ...
    [[nodiscard]] static expected<ComplianceReport> generate_article13(
        ...
        for (const auto& path : inference_log_files) {
            ...
            if (!all) {
                if (opts.strict_source_reads) {
                    return Error{ErrorCode::IO_ERROR,
                        "EUAIActReporter Art.13: failed to read '" + path +
                        "': " + all.error().message};
                }
                ++read_errors;
                continue;
    ...
    [[nodiscard]] static expected<ComplianceReport> generate_article19(
        ...
        for (const auto& path : decision_log_files) {
            ...
            if (!all) {
                if (opts.strict_source_reads) {
                    return Error{ErrorCode::IO_ERROR,
                        "EUAIActReporter Art.19: failed to read decision log '" + path +
                        "': " + all.error().message};
                }
                ++dec_read_errors;
                continue;
        ...
        for (const auto& path : inference_log_files) {
            ...
            if (!all) {
                if (opts.strict_source_reads) {
                    return Error{ErrorCode::IO_ERROR,
                        "EUAIActReporter Art.19: failed to read inference log '" + path +
                        "': " + all.error().message};
                }
                ++inf_read_errors;
                continue;
