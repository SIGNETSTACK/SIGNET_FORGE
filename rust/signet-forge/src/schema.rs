...
/// Lifetime-bound view of a schema borrowed from a [`crate::ParquetReader`].
///
/// The compiler enforces that this value cannot outlive the reader it came
/// from, preventing the use-after-free that an unconstrained [`Schema`] copy
/// would permit.  All accessor methods are identical to those on [`Schema`].
...
// SAFETY: SchemaRef is Send for the same reason.
unsafe impl<'a> Send for SchemaRef<'a> {}
