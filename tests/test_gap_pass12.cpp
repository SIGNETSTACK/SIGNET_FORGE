...
TEST_CASE("ThreatModelAnalyzer: default model has all 6 STRIDE categories",
    ...
    auto analysis = ThreatModelAnalyzer::analyze(model);
...
TEST_CASE("ThreatModelAnalyzer: incomplete model flags missing categories",
    ...
    auto analysis = ThreatModelAnalyzer::analyze(m);
    REQUIRE_FALSE(analysis.stride_complete);
    REQUIRE(analysis.missing_categories.size() == 5);
}
...
TEST_CASE("ThreatModelAnalyzer: mitigation status tracking",
    ...
    REQUIRE(analysis.mitigated_count == 6);
    REQUIRE(analysis.unmitigated_count == 0);
}
